...
class RegistrationCheckApi
{
     ...
     *
     * @param  \Swagger\Client\Model\RegistrationCheckRequestV2Model $model model (required)
     *
    ...
    {
        list($response) = $this->registrationCheckV2PostWithHttpInfo($model);
        return $response;
    }

    /**
     * Operation registrationCheckV2PostWithHttpInfo
     *
     * # Chamber of Commerce registration check  If you sell to businesses, the shop can do a registration check of the corporation’s chamber of commerce number. The API will return the address data if the number is found.  This option may or may not be available, dependent on your contract with Capayable, as it entails an extra cost.     Note: the CoC check on the live environment may take a while, so be sure to set longer timeouts (max 2 minutes).    Supported countries:  - Test environment: NL  - Live environment: NL
     *
     ...
     */
    public function registrationCheckV2PostWithHttpInfo($model)
    {
        $returnType = '\Swagger\Client\Model\RegistrationCheckResultViewModel';
        $request = $this->registrationCheckV2PostRequest($model);
    ...
    /**
     * Operation registrationCheckV2PostAsync
     *
     * # Chamber of Commerce registration check  If you sell to businesses, the shop can do a registration check of the corporation’s chamber of commerce number. The API will return the address data if the number is found.  This option may or may not be available, dependent on your contract with Capayable, as it entails an extra cost.     Note: the CoC check on the live environment may take a while, so be sure to set longer timeouts (max 2 minutes).    Supported countries:  - Test environment: NL  - Live environment: NL
     *
     * @param  \Swagger\Client\Model\RegistrationCheckRequestV2Model $model (required)
     ...
     */
    public function registrationCheckV2PostAsync($model)
    {
        return $this->registrationCheckV2PostAsyncWithHttpInfo($model)
            ->then(
                function ($response) {
                    return $response[0];
                }
    ...
    /**
     * Operation registrationCheckV2PostAsyncWithHttpInfo
     *
     * # Chamber of Commerce registration check  If you sell to businesses, the shop can do a registration check of the corporation’s chamber of commerce number. The API will return the address data if the number is found.  This option may or may not be available, dependent on your contract with Capayable, as it entails an extra cost.     Note: the CoC check on the live environment may take a while, so be sure to set longer timeouts (max 2 minutes).    Supported countries:  - Test environment: NL  - Live environment: NL
     *
     * @param  \Swagger\Client\Model\RegistrationCheckRequestV2Model $model (required)
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function registrationCheckV2PostAsyncWithHttpInfo($model)
    {
        $returnType = '\Swagger\Client\Model\RegistrationCheckResultViewModel';
        $request = $this->registrationCheckV2PostRequest($model);
    ...
    /**
     * Create request for operation 'registrationCheckV2Post'
     *
     * @param  \Swagger\Client\Model\RegistrationCheckRequestV2Model $model (required)
     *
     * @throws \InvalidArgumentException
     ...
     */
    protected function registrationCheckV2PostRequest($model)
    {
        // verify the required parameter 'model' is set
        if ($model === null || (is_array($model) && count($model) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $model when calling registrationCheckV2Post'
            );
        }
